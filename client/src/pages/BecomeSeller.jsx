        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="flex flex-col gap-8 bg-gray-50 p-6 rounded-lg border border-gray-100">
            {form.sellerType === "store" && (
              <div className="mb-6">
                <h3 className="text-lg font-semibold text-gray-700 mb-3">Store Details</h3>
                <input name="storeName" value={form.storeName} onChange={handleChange} placeholder="Store Name (optional)" className="border p-2 rounded mb-3 w-full" />
                <textarea name="storeDescription" value={form.storeDescription} onChange={handleChange} placeholder="Store Description (optional)" className="border p-2 rounded mb-3 w-full" />
                <input type="file" name="storeLogo" onChange={handleChange} className="border p-2 rounded mb-3 w-full" accept="image/*" />
              </div>
            )}
            <div className="mb-6">
          ...
          <div className="flex flex-col gap-8 bg-gray-50 p-6 rounded-lg border border-gray-100">
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-3">Location & Security</h3>
              <input name="zip" value={form.zip} onChange={handleChange} placeholder="ZIP" className="border p-2 rounded mb-3 w-full" required />
              <input name="state" value={form.state} onChange={handleChange} placeholder="State" className="border p-2 rounded mb-3 w-full" required />
              <input name="country" value={form.country} onChange={handleChange} placeholder="Country" className="border p-2 rounded mb-3 w-full" required />
              <input name="password" value={form.password} onChange={handleChange} placeholder="Password" className="border p-2 rounded mb-3 w-full" type="password" required />
              <input name="profilePic" value={form.profilePic} onChange={handleChange} placeholder="Profile Picture URL (optional)" className="border p-2 rounded mb-3 w-full" />
            </div>
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-3">Payout Methods</h3>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-700 mb-3">KYC & Tax Information</h3>
              <select name="taxForm" value={form.taxForm} onChange={handleChange} className="border p-2 rounded mb-3 w-full" required>
                <option value="">Select Tax Form</option>
                <option value="W-9">W-9 (US)</option>
                <option value="W-8BEN">W-8BEN (Non-US)</option>
              </select>
              <input name="taxId" value={form.taxId} onChange={handleChange} placeholder="Tax ID (SSN/ITIN/EIN or equivalent)" className="border p-2 rounded mb-3 w-full" required />
              <input name="dob" value={form.dob} onChange={handleChange} type="date" placeholder="Date of Birth" className="border p-2 rounded mb-3 w-full" required />
              <input name="legalTaxAddress" value={form.legalTaxAddress} onChange={handleChange} placeholder="Legal/Tax Address" className="border p-2 rounded mb-3 w-full" required />
              <label className="block mb-2">Government ID Upload (passport/driver’s license)</label>
              <input type="file" name="govId" onChange={handleChange} className="border p-2 rounded mb-3 w-full" accept="image/*,.pdf" />
